...
export default function FlowithFooter() {
  ...
  useEffect(() => {
    ...
    class FluidSimulation {
      ...
      resizeCanvas() {
        ...
        const ratio = rect.width / rect.height;
        this.res = {
          w: Math.max(256 * ratio, rect.width * dpr * 0.5),
          h: Math.max(256, rect.height * dpr * 0.5),
        };
      }
      ...
      createProgram(fragSource) {
        ...
        const fragShader = this.createShader(fragSource, gl.FRAGMENT_SHADER);
        const program = gl.createProgram();
        gl.attachShader(program, vertShader); gl.attachShader(program, fragShader);
        gl.linkProgram(program);
      ...
      createShaders() {
        ...
        this.displayProgram = this.createProgram(`
          ...
          void main () {
            ...
            velocity += .001; vec2 img_uv = vUv;
            img_uv -= u_disturb_power * normalize(velocity) * offset; img_uv -= u_disturb_power * normalize(velocity) * offset;
            /* Apply the offset to the image coordinates */
            vec4 bgColor = ${backgroundColor};
            if (img_uv.x < 0.0 || img_uv.x > 1.0 || img_uv.y < 0.0 || img_uv.y > 1.0) { gl_FragColor = bgColor; } 
            else { vec4 img = texture2D(u_text_texture, vec2(img_uv.x, 1. - img_uv.y)); gl_FragColor = img; }
          }`);
      ...
      createDoubleFBO(w, h) {
        let fbo1 = this.createFBO(w, h); let fbo2 = this.createFBO(w, h);
        return {
          width: w, height: h, texelSizeX: 1 / w, texelSizeY: 1 / h,
          read: () => fbo1, write: () => fbo2, swap: () => { [fbo1, fbo2] = [fbo2, fbo1]; },
        };
      }

      initFBOs() {
        const w = Math.floor(this.res.w); const h = Math.floor(this.res.h);
        this.outputColor = this.createDoubleFBO(w, h); this.velocity = this.createDoubleFBO(w, h);
        this.divergence = this.createFBO(w, h); this.pressure = this.createDoubleFBO(w, h);
      }

      loadTexture() {
        const gl = this.gl;
        const scale = Math.min(window.devicePixelRatio || 1, 2) * 2;
        const texWidth = Math.min(this.canvas.width * scale, 3000); 
        const texHeight = Math.min(this.canvas.height * scale, 2048);
        const isDark = this.themeMode === "dark";
        
        const background = isDark ? "#08090b" : "#F6F7F8";
        // INCREASED BRIGHTNESS: Opacity bumped from 0.15 to 0.35/0.4
        // const textColor = isDark ? "rgba(255, 255, 255, 0.35)" : "rgba(0, 0, 0, 0.4)";
        const textColor = isDark ? "rgba(200, 200, 200, 0.85)" : "rgba(0, 0, 0, 0.4)";
        ...
        // MASSIVE SCALING: Using width to calculate font size to span horizontally
        const fontSize = Math.min(texWidth * 0.18, texHeight * 0.6);
        ctx.font = `900 ${fontSize}px system-ui, -apple-system, sans-serif`;
        
        // Centered exactly in the middle of the canvas
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        // Stretched text appearance slightly by adjusting tracking if needed (native Canvas doesn't easily support letter-spacing, but font-weight 900 helps)
        ctx.fillText("AAICY", texWidth / 2, texHeight / 2); 
      ...
      setupEvents() {
        ...
        this.handleResize = () => {
          clearTimeout(resizeTimeout);
          resizeTimeout = setTimeout(() => {
            this.resizeCanvas(); this.initFBOs(); this.loadTexture();
          }, 150); 
        };
      ...
      render() {
        if (!this.isRendering) return;
        if (!this.imageTexture) {
          this.animationFrame = requestAnimationFrame(this.render.bind(this)); return;
        }

        const gl = this.gl;
        const dt = 1 / 60; const canvasWidth = this.canvas.width; const canvasHeight = this.canvas.height;
        const dpr = this.dpr || 1; const cssWidth = canvasWidth / dpr; const cssHeight = canvasHeight / dpr;
        ...
        if (this.pointer.moved) {
          ...
          gl.uniform1i(this.splatProgram.uniforms.u_input_texture, this.outputColor.read().attach(1));
          gl.uniform3f(this.splatProgram.uniforms.u_point_value, this.params.cursorPower * 0.001, 0, 0);
          this.blit(this.outputColor.write()); this.outputColor.swap();
        }
  ...
  return (
    ...
    >
      ...
      {/* Foreground Interactive DOM Layer */}
      <div className="relative z-10 w-full h-full flex flex-col items-center justify-between pointer-events-none pt-12 pb-6 px-4">
        
        {/* Call to Action Container - Pushed to the top */}
        <div className="flex flex-col items-center max-w-2xl text-center space-y-4">
          {/* <h2 className="text-4xl md:text-5xl font-semibold tracking-tight text-black dark:text-white pointer-events-auto">
            Build smarter. Scale faster.
          </h2> */}
          {/* <p className="text-lg md:text-xl text-gray-700 dark:text-gray-300 font-medium">
            Stop losing time to disconnected workflows. Let's engineer automated, full-stack systems that elevate your business.
          </p> */}
        </div>

        {/* Footer Area - Pushed to the bottom */}
        <div className="flex flex-col items-center space-y-6 w-full">
          {/* Social Icons Layout - Horizontal & Professional */}
          <div className="flex gap-8 pointer-events-auto">
            <SocialIcon href="https://www.linkedin.com/company/aaicy" label="LinkedIn">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z" />
              </svg>
            </SocialIcon>

            <SocialIcon href="https://youtube.com/@AiandCodewithYar" label="YouTube">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <SocialIcon href="https://www.instagram.com/aiandcodewithyar/" label="Instagram">
               <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
